...
function App() {
	...
	React.useEffect(() => {
		...
		fetch(
			`https://63e50fddc04baebbcdb2d329.mockapi.io/photo_collections?page=${page}&limit=3&${category}` 
		)
		.then(res => res.json())
		.then(json => {
			setCollections(json)
		...
		.catch((err) => {
			console.warn(err);
			alert('Error fetching photo collections');
		}).finally(() => {
			setIsLoading(false);
		})
	}, [categoryId, page]);
